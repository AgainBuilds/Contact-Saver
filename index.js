/**
 * MATCH Contact Bot - FINAL Pairing Code Version
 */
...
async function startBot() {
  ...
  // Pairing Code (key part that worked)
  if (!state.creds.registered) {
    ...
    try {
      ...
      console.log('Tell owner to use this code in WhatsApp Linked Devices → Link with phone number');
      console.log('================================\n');
    } catch (e) {
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection === 'close') {
      ...
      if (shouldReconnect) setTimeout(() => startBot(), 6000);
    }
  });
  ...
  sock.ev.on('messages.upsert', async ({ messages }) => {
    ...
    try {
      ...
      await sock.sendMessage(ownJid, { text: `✅ Exported ${count} contacts!` });
    } catch (err) {
